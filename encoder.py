...
class HierarchicalAttentionEncoder(nn.Module, Encoder):
    """ Multi-scale attention that processes semantic features at different levels """
    def __init__(self, input_dim, output_dim, num_heads=None, seed=3):
        ...
        scales = [input_dim // 4, input_dim // 2, input_dim]
        
        # Calculate valid num_heads for each scale
        base_num_heads = calculate_optimal_num_heads(input_dim, preferred_heads=num_heads)
        self.scale_num_heads = []
        
        for scale in scales:
            if scale <= 0:
                scale = max(1, input_dim // 8)  # Fallback for very small scales
            
            # Find valid number of heads for this scale, prefer smaller numbers for smaller scales
            scale_heads = calculate_optimal_num_heads(scale, max_heads=base_num_heads)
            self.scale_num_heads.append(scale_heads)
        
        # Ensure scales are valid
        scales = [max(1, scale) for scale in scales]
        ...
        # Attention at different scales with dynamically calculated heads
        self.scale_attentions = nn.ModuleList([
            nn.MultiheadAttention(scale, self.scale_num_heads[i], batch_first=True)
            for i, scale in enumerate(scales)
        ])
        
        # Layer norms for each scale
        self.scale_norms = nn.ModuleList([
            nn.LayerNorm(scale) for scale in scales
        ])
        
        # Combine different scales
        total_features = sum(scales)
    ...
    def forward(self, x):
        ...
        for proj, attn, norm in zip(self.scale_projections, self.scale_attentions, self.scale_norms):
            # Project to scale
            x_scale = proj(x).unsqueeze(1)  # Add sequence dimension
            
            # Apply attention
            attn_out, _ = attn(x_scale, x_scale, x_scale)
            x_scale = norm(x_scale + attn_out)
            
            # Remove sequence dimension and store
            scale_outputs.append(x_scale.squeeze(1))
    ...
    def learn_encoder(self, voxels, vectors, epochs=3000, learning_rate=0.0004):
...
class EnsembleEncoder(Encoder):
    """ Ensemble of multiple encoders for improved robustness. """
    def __init__(self, input_dim, output_dim, seed=3):
        self.seed = seed
        np.random.seed(seed)
        torch.manual_seed(seed)
        
        # Create multiple different encoders
        self.encoders = []
        
        # Add FC2 encoders with different seeds
        for i in range(3):
            self.encoders.append(FC2Encoder(input_dim, output_dim, seed=seed+i))
        
        # Add a residual encoder to the mix
        self.encoders.append(ResidualEncoder(input_dim, output_dim, seed=seed+10))

    def learn_encoder(self, voxels, vectors, epochs=1500, learning_rate=0.001):
        """ Train all encoders in the ensemble. """
        for i, encoder in tqdm(enumerate(self.encoders), total=len(self.encoders), desc="Training Ensemble Encoders", unit="encoder", leave=False):
            #print(f"Training encoder {i+1}/{len(self.encoders)}")
            encoder.learn_encoder(voxels, vectors, epochs=epochs, learning_rate=learning_rate)

    def encode(self, vectors):
        """ Encode using ensemble averaging. """
        predictions = []
        for encoder in self.encoders:
            pred = encoder.encode(vectors)
...
class MixtureOfExpertsEncoder(nn.Module, Encoder):
    """ Mixture of Experts encoder that routes different inputs to different expert networks. """
    def __init__(self, input_dim, output_dim, num_experts=4, seed=3):
        super(MixtureOfExpertsEncoder, self).__init__()
        self.seed = seed
        np.random.seed(seed)
        torch.manual_seed(seed)
    ...
    def forward(self, x):
        ...
        # Weight expert outputs by gate weights
    ...
    def learn_encoder(self, voxels, vectors, epochs=2000, learning_rate=0.001):
        """ Learn the mixture of experts encoder. """
        criterion = nn.MSELoss()
        optimizer = optim.AdamW(self.parameters(), lr=learning_rate, weight_decay=1e-4)
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
...
class BottleneckEncoder(nn.Module, Encoder):
    """ Encoder with information bottleneck to learn compressed representations. """
    def __init__(self, input_dim, output_dim, bottleneck_dim=None, seed=3):
        ...
        if bottleneck_dim is None:
            bottleneck_dim = min(input_dim, output_dim) // 4
        ...
        self.info_linear = nn.Linear(bottleneck_dim, bottleneck_dim)

    def forward(self, x):
        # Encode to bottleneck
        z = self.encoder(x)
        
        # Add information bottleneck noise during training
        if self.training:
            # Add controlled noise to create information bottleneck
            noise = torch.randn_like(z) * 0.1
            z = z + noise
        
        # Apply information bottleneck transformation
        z = self.info_linear(z)
        z = nn.ReLU()(z)
    ...
    def learn_encoder(self, voxels, vectors, epochs=2000, learning_rate=0.001):
        """ Learn the bottleneck encoder. """
        criterion = nn.MSELoss()
        optimizer = optim.AdamW(self.parameters(), lr=learning_rate, weight_decay=1e-4)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=200)
